...
- В модели `BasePage` и `BaseComponent` добавлено поле `is_deleted` и менеджер `available_objects`.
- Для страниц и компонентов реализован функционал мягкого удаления.
